...
| Rollback targets | `e096dfbf` (AI authz), `4d641086` (pre-hardening) |
...
9. **JWT review** — reviewed. Secret is env-only (`JWT_SECRET`), HS256, `fromAuthHeaderAsBearerToken`, ~8h expiry, no refresh token, JWT stored in browser localStorage. Inactive/deleted-user rejection **now enforced on all AI endpoints** via `ScreenAuthzService`. Global `JwtStrategy` inactive-user rejection recommended as a small follow-up (adds a per-request DB lookup — deferred to control blast radius).
10. **Inactive/deleted user handling** — ✅ on AI/data-egress endpoints (via `ScreenAuthzService`); ⚠️ not yet global.
...
17. **API authorization audit** — ✅ `FINAL_API_AUTHORIZATION_AUDIT.md` (A/B/C/D). AI endpoints → A; most CRUD → B (JWT-only) with scoped follow-up recommended; `/api/auth/seed` flagged for review.
...
28. **P1** — global JWT inactive-user rejection not yet enforced (mitigated on sensitive endpoints); broad CRUD endpoints authenticated-only.
...
| No secrets committed | ❌ **P0 service_role key + DB password in source** |
| No P0 security defects | ❌ 2 open (credential exposure) |
