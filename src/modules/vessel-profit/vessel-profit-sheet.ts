...
export interface SheetVoyage {
  ...
  /*
   * وسمُ الدفتر نفسه: «⚠️ راجعها».
   *
   * ── ولماذا يُحمَل ولا يُصلَح ──
   * الدفتر يضع هذا الوسم حين يشكّ في صفٍّ، والحقل كان يصل الحمولة **ولا يقرؤه
   * أحد**. فرحلة بيلاجوس `#2` في ٢ يناير ٢٠٢٦ سيولتها صفرٌ وصافيها سالب، وهي
   * موسومةٌ في الدفتر — والشاشة تجمع الصفر بأمانةٍ تامّة ولا تقول شيئاً.
   *
   * فاكتُشف النقص بعين المالك لا بالنظام: ١٦٧٬٥٥٣ ناقصةً من سيولة يناير.
   *
   * ── والسيولة لا تُشتقّ ──
   * الصافي يُحسب من مكوّناته فالشاشة محصَّنةٌ منه. أمّا السيولة فمفهومٌ مستقلٌّ
   * لا يتبع الصافي — قارنّا فوجدناها تعلوه وتقلّ عنه بلا قاعدة. فاختراعُ رقمٍ
   * لها كذبٌ في دفترٍ ماليّ. والعلاج أن يُرى الوسم، لا أن يُملأ الفراغ.
   */
  broken: boolean;
}
...
export function toSheetVoyage(p: any, spec: SheetVesselSpec): SheetVoyage {
  ...
  return {
    ...
    // `true` وحدها وسمٌ — وغيابُ الحقل في حمولةٍ قديمة ليس وسماً
